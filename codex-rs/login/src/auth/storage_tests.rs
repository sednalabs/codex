...
fn auto_repository_falls_back_only_after_a_positive_empty_keyring_read() -> anyhow::Result<()> {
    let codex_home = tempdir()?;
    let mock_keyring = MockKeyringStore::default();
    let failing_store: Arc<dyn KeyringStore> = Arc::new(FailingSaveKeyringStore {
        inner: mock_keyring,
    });
    let repository = create_auth_repository_with_store(
        codex_home.path().to_path_buf(),
    ...
    let loaded = repository.replace_for_login(&replacement)?;
    assert_eq!(loaded.source, AuthStorageSource::File);
    assert_eq!(
