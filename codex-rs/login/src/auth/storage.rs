...
///
/// The lock is held across policy selection and compare/update operations.
/// Individual stores never use it and never perform cross-store cleanup, which
/// keeps a captured `LoadedAuth` a real authority boundary.
#[derive(Clone, Debug)]
...
/// keyed by canonical CODEX_HOME and remains held across complete authority
/// transactions in every process sharing that directory.
...
impl AuthRepository {
            ...
            AuthCredentialsStoreMode::Auto => {
                ...
                match self.storage(keyring_source).load()? {
                    ...
                    None => match self.storage(keyring_source).save(auth) {
                        Ok(()) => keyring_source,
                        Err(err) => match self.storage(keyring_source).load()? {
                            None => {
                                warn!(
    ...
    /// User-initiated logout is deliberately independent of configured policy.
    /// All stores are attempted even after one fails.
...
fn auth_repository_lock_path(codex_home: &Path) -> std::io::Result<PathBuf> {
    // The authority lock lives inside CODEX_HOME, not under the caller's
    // temporary directory. Canonicalizing when possible makes aliases share
    // an identity; the absolute path fallback is still deterministic when the
    // home has not been created yet.
    let canonical_home = codex_home
        .canonicalize()
        .unwrap_or_else(|_| codex_home.to_path_buf());
    Ok(canonical_home.join(".auth-repository.lock"))
}
