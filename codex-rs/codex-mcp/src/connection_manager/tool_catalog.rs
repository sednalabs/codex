...
impl McpConnectionSet {
        ...
        for (server_name, managed_client) in &self.clients {
            ...
            let Some(server_tools) = async {
                match catalog_override {
                    ...
                    None => {
                        // A published cache, including an intentionally empty catalogue, is the
                        // only nonblocking inventory while startup is pending. Refreshing first
                        // would await the startup future and make that cache unreachable.
    ...
    pub(crate) async fn capture_binding_with_metadata(
        ...
        for managed_client in self.clients.values() {
            catalog_changed |= managed_client.refresh_tools_if_changed().await;
        }
        if catalog_changed {
            *self.tool_catalog_revision.write().await += 1;
        }
