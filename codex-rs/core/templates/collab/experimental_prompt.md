...
* Prefer long blocking waits with `wait_agent`; use the maximum useful `timeout_ms` and avoid repeated short waits. V2 wait is event-driven and wakes on terminal child events or mailbox activity, so do not poll `list_agents` or `inspect_agent_tree` while the awaited state is unchanged. Use `return_when=any` to unblock on the first terminal sub-agent and `return_when=all` when every sub-agent must finish. If a mailbox wake does not satisfy the requested terminal condition, rearm the same target set and return rule while preserving the wait contract. Only block when you need the transition to complete or when redispatching.
