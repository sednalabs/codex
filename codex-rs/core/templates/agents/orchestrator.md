...
3. Inspect in-flight status with `list_agents` for the cheap live view; use its active-subagent signal to decide when deeper inspection is worth it. Use `inspect_agent_tree` when you need a compact nested subtree view, stale-descendant visibility, or branch-focused inspection with `agent_roots`. Then call `wait_agent` only when you must block for a transition to finish. Use the default `return_when=any` to unblock on the first terminal child and `return_when=all` when you need every child to be terminal; prefer long timeouts so you do not spin on repeated short waits, and inspect `pending_ids` plus `completion_reason` when the call returns. For helper-backed waits (for example, `exec_command` / `write_stdin`), set `wait_until_terminal=true` so the tool layer absorbs the wait instead of transcript polling.
