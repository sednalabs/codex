...
3. Call `wait_agent` directly when you must block for a transition to finish. V2 wait is event-driven and wakes on terminal child events or mailbox activity; do not poll `list_agents` or `inspect_agent_tree` while the awaited state is unchanged. Use the default `return_when=any` to unblock on the first terminal child and `return_when=all` when you need every child to be terminal; prefer the maximum useful timeout, and inspect `pending_ids` plus `completion_reason` when the call returns. If a mailbox wake does not satisfy the requested terminal condition, rearm the same target set and return rule while preserving the wait contract.
