...
impl ModelVisibleAgentIdentity {
    ...
    pub(crate) fn from_stored(
        ...
        let configured_identity = ModelVisibleConfiguredIdentity {
            ...
            service_tier: bounded_optional(
                stored.configured_service_tier.as_deref(),
                encoding,
                &mut truncated,
            ),
        ...
        let latest_turn_request_identity =
            stored.latest_turn_request_identity.as_ref().map(|turn| {
                ModelVisibleTurnRequestIdentity {
                    turn_id: bounded_optional(turn.turn_id.as_deref(), encoding, &mut truncated),
                    model: bounded_optional(
                        Some(turn.request_model.as_str()),
                        encoding,
                        &mut truncated,
                    ...
                    model_provider_id: bounded_optional(
                        turn.model_provider_id.as_deref().and_then(non_empty),
                        encoding,
                        &mut truncated,
                    ),
                    reasoning_effort: turn.requested_reasoning_effort.clone(),
