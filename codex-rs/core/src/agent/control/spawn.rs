...
/// Browser tools are session-scoped and deliberately do not persist across a resume. Resolve
/// them from the child configuration at every AgentControl lifecycle boundary instead of relying
/// on the ambient process home or persisted rollout metadata.
