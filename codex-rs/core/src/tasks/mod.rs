...
pub(crate) trait SessionTask: Send + Sync + 'static {
    ...
    /// Returns whether successful completion may atomically claim explicit same-turn input.
    ///
    /// This is opt-in because a continuation must preserve the task's own lifecycle and
    /// state; task kind alone is not enough to establish those invariants.
    fn supports_turn_local_continuation(&self) -> bool {
        false
    }

    /// Runs a continuation without emitting a second task-start lifecycle.
...
impl Session {
    ...
    async fn start_task_impl<T: SessionTask>(
        ...
        let handle = tokio::spawn(
            async move {
                ...
                let task_input = 'task: {
                    if tokio::select! {
                        _ = task_cancellation_token.cancelled() => true,
                        published = published_rx => published.is_err(),
                    } {
                        break 'task None;
                    }
                    ...
                    let token_usage_at_turn_start = tokio::select! {
                        _ = task_cancellation_token.cancelled() => None,
                        token_usage = lifecycle => Some(token_usage),
                    };
                    let Some(token_usage_at_turn_start) = token_usage_at_turn_start else {
                        break 'task None;
                    };
                    let handoff = async {
                        let mut transfer = sess.input_queue.pending_turn_input_transfer().await;
                        let _transition = sess.input_queue.lock_task_transition().await;
                        let mut active = sess.active_turn.lock().await;
                        let active_turn = active.as_mut()?;
                        let mut turn_state = active_turn.turn_state.lock().await;
                        let running_task = active_turn.task.as_mut()?;
                        if running_task.identity != task_identity
                            || running_task.phase != RunningTaskPhase::Preparing
                            || task_cancellation_token.is_cancelled()
                            || running_task.initial_input.is_none()
                        {
                            return None;
                        }
                    ...
                    let Some((transfer, task_input)) = (tokio::select! {
                        _ = task_cancellation_token.cancelled() => None,
                ...
                if !task_cancellation_token.is_cancelled() {
                    ...
                    // The finish path may atomically claim explicit same-turn input and invoke
                    // the same task again without publishing a second turn lifecycle.
                    sess.on_task_finished(task_identity, Arc::clone(&ctx_for_finish), task_result)
                        .await;
                }
                done_clone.notify_waiters();
            }
    ...
    pub async fn abort_all_tasks(self: &Arc<Self>, reason: TurnAbortReason) {
        self.abort_tasks(reason, /*restart_pending_work*/ true)
            .await;
    }
    ...
    pub async fn on_task_finished(
        ...
        loop {
            ...
            // The decision above is the only operation performed while the task-transition,
            // active-turn, turn-state, and terminal-error locks are held. Run the continuation
            // after all of them have been released so steering cannot deadlock on this path.
    ...
    /// Atomically claims explicit turn-local input for a same-task continuation, or leaves all
    /// state untouched when the task is stale or the input is not eligible.
    #[expect(
    ...
    async fn handle_task_abort(self: &Arc<Self>, mut task: RunningTask, reason: TurnAbortReason) {
        ...
        task.turn_context
            .turn_metadata_state
            .cancel_git_enrichment_task();
        let preparing_input = (task.kind == TaskKind::Regular
            && task.phase == RunningTaskPhase::Preparing)
            .then(|| task.initial_input.take())
            .flatten();
        ...
        if let Some(input) = preparing_input {
            run_hooks_and_record_inputs(self, &task.turn_context, &input).await;
        }
            ...
            )
        {
            ...
            if self.flush_rollout().await.is_err() {
                warn!("failed to flush rollout after task abort marker");
            }
        }
