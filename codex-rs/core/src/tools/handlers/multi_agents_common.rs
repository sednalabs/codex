...
pub(crate) fn normalize_spawn_agent_reasoning_effort(
    config: &mut Config,
    model: &str,
    supported_reasoning_levels: &[ReasoningEffortPreset],
    ...
    match config.model_reasoning_effort {
        ...
        Some(reasoning_effort) => {
            ...
            config.model_reasoning_effort = default_reasoning_effort;
            Ok(())
        }
        None => {
            config.model_reasoning_effort = default_reasoning_effort;
...
mod tests {
    ...
    fn supported_reasoning_levels(efforts: &[ReasoningEffort]) -> Vec<ReasoningEffortPreset> {
        efforts
            .iter()
            .copied()
    ...
    #[tokio::test]
    async fn normalize_spawn_agent_reasoning_effort_falls_back_for_role_defaults() {
        ...
        normalize_spawn_agent_reasoning_effort(
            &mut config,
            "fallback-model",
            &supported_reasoning_levels(&[ReasoningEffort::Low]),
            Some(ReasoningEffort::Low),
            /*requested_reasoning_effort*/ None,
        )
        .expect("role/default reasoning should fall back to the model default");

        assert_eq!(config.model_reasoning_effort, Some(ReasoningEffort::Low));
    }

    #[tokio::test]
    async fn normalize_spawn_agent_reasoning_effort_rejects_unsupported_requested_value() {
        let mut config = ConfigBuilder::default()
            .build()
            .await
            .expect("load default config");
        ...
        let err = normalize_spawn_agent_reasoning_effort(
            &mut config,
            "fallback-model",
            &supported_reasoning_levels(&[ReasoningEffort::Low]),
            Some(ReasoningEffort::Low),
        ...
        .expect_err("explicit requested reasoning should still validate strictly");

        assert_eq!(
            err,
            FunctionCallError::RespondToModel(
                "Reasoning effort `medium` is not supported for model `fallback-model`. Supported reasoning efforts: low".to_string()
            )
        );
    }
}
