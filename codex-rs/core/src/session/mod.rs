...
pub(crate) use self::handlers::inter_agent_communication;
#[cfg(test)]
...
impl Session {
    ...
    async fn spawn_internal(args: SessionSpawnArgs) -> CodexResult<(Arc<Self>, SessionIo)> {
        ...
        // fall back to rollout-file tools. The active child may supply freshly-derived Browser
        // tools, but those must not suppress unrelated persistent tools from the thread.
        ...
        // Freshly-derived Browser tools replace any stale Browser entries, while unrelated
...
impl Session {
    ...
    pub async fn notify_computer_use_response(&self, call_id: &str, response: ComputerUseResponse) {
        ...
        match entry {
            ...
            None => {
                warn!("No pending computer-use call found for call_id: {call_id}");
            }
        }
    }
