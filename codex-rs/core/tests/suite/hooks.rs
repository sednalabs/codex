...
async fn permission_request_hook_allows_shell_command_without_user_approval() -> Result<()> {
    ...
    let marker = "permissionrequest-shell-command-marker";
    let command = format!("rm -f {marker}");
...
async fn permission_request_hook_sees_raw_exec_command_input() -> Result<()> {
    ...
    let marker = "permissionrequest-exec-command-marker";
    let command = format!("rm -f {marker}");
...
async fn pre_tool_use_blocks_shell_command_before_execution() -> Result<()> {
    ...
    let marker = "pretooluse-shell-command-marker";
...
async fn plugin_pre_tool_use_blocks_shell_command_before_execution() -> Result<()> {
    ...
    let marker = "plugin-pretooluse-shell-command-marker";
    let command = format!("printf blocked > {marker}");
    let args = serde_json::json!({ "command": command });
    let responses = mount_sse_sequence(
        &server,
...
"#,
  ...
  "hooks": {
    ...
    if marker_path.exists() {
        fs::remove_file(&marker_path).context("remove leftover plugin pre tool use marker")?;
    }
...
async fn pre_tool_use_blocks_shell_when_defined_in_config_toml() -> Result<()> {
    ...
    let marker = "pretooluse-config-toml-marker";
    let command = format!("printf blocked > {marker}");
    let args = serde_json::json!({ "command": command });
    let responses = mount_sse_sequence(
        &server,
    ...
    if marker_path.exists() {
        fs::remove_file(&marker_path).context("remove leftover config.toml marker")?;
    }
...
async fn pre_tool_use_blocks_local_shell_before_execution() -> Result<()> {
    ...
    let marker = "pretooluse-local-shell-marker";
    let command = vec![
        "/bin/sh".to_string(),
        "-c".to_string(),
        format!("printf blocked > {marker}"),
    ];
    let responses = mount_sse_sequence(
        &server,
        vec![
    ...
    if marker_path.exists() {
        fs::remove_file(&marker_path).context("remove leftover local shell marker")?;
    }
...
async fn pre_tool_use_blocks_exec_command_before_execution() -> Result<()> {
    ...
    let marker = "pretooluse-exec-command-marker";
    let command = format!("printf blocked > {marker}");
    ...
    if marker_path.exists() {
        fs::remove_file(&marker_path).context("remove leftover exec marker")?;
    }
    ...
    assert!(
        !marker_path.exists(),
        "blocked exec command should not execute"
    );
