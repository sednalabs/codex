...
impl<T: HttpTransport> ResponsesClient<T> {
    ...
    /// Streams a request while invoking `observer` at each physical retry
    /// attempt immediately before the underlying HTTP transport is called.
    pub async fn stream_request_with_observer(
        ...
        self.stream_encoded_observed(body, headers, compression, turn_state, observer)
            .await
    }
    ...
    async fn stream_encoded_observed(
        &self,
        body: EncodedJsonBody,
        extra_headers: HeaderMap,
        compression: Compression,
        ...
        let stream_response = self
            .session
            .stream_encoded_json_with_observer(
                Method::POST,
                Self::path(),
                extra_headers,
                Some(body),
