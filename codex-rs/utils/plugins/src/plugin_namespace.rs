...
pub type PluginNamespaceFuture<'a> = Pin<Box<dyn Future<Output = Option<String>> + Send + 'a>>;
...
pub fn plugin_namespace_for_root_uri<'a>(
    fs: &'a dyn ExecutorFileSystem,
    plugin_root: &'a PathUri,
) -> PluginNamespaceFuture<'a> {
    Box::pin(async move {
...
pub fn plugin_namespace_for_skill_path<'a>(
    fs: &'a dyn ExecutorFileSystem,
    path: &'a AbsolutePathBuf,
) -> PluginNamespaceFuture<'a> {
    Box::pin(async move {
        let path_uri = PathUri::from_abs_path(path);
        plugin_namespace_for_skill_uri(fs, &path_uri).await
    })
}

/// Returns the plugin manifest `name` for the nearest URI ancestor of `path`.
pub fn plugin_namespace_for_skill_uri<'a>(
    fs: &'a dyn ExecutorFileSystem,
    path: &'a PathUri,
) -> PluginNamespaceFuture<'a> {
    Box::pin(async move {
