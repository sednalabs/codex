...
async fn override_turn_context_preserves_full_access_permissions_without_active_profile() {
    Box::pin(async {
        ...
        assert_eq!(
            notification.thread_settings.sandbox_policy,
            codex_app_server_protocol::SandboxPolicy::DangerFullAccess,
        );
        assert_eq!(notification.thread_settings.active_permission_profile, None);
        assert_eq!(
        ...
        let updated_session = app
            ...
            .as_ref()
            .expect("primary session should be updated from notification");
        assert_eq!(updated_session.approval_policy, AskForApproval::Never);
        assert_eq!(
            updated_session.permission_profile,
            PermissionProfile::Disabled
        );
        assert_eq!(updated_session.active_permission_profile, None);
        assert_eq!(
            updated_session
                .collaboration_mode
                .as_deref()
