...
impl std::fmt::Display for LoadedThreadPageRejection {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::InvalidThreadId => f.write_str("invalid loaded-thread id"),
            Self::DuplicateThreadId => f.write_str("duplicate loaded-thread id"),
            Self::MalformedCursor => f.write_str("malformed loaded-thread pagination cursor"),
        }
    }
}
...
mod loaded_thread_page_tests {
    ...
    fn rejects_malformed_ids_and_cursors_before_backfill() {
        ...
        assert_eq!(
            ...
            Err(LoadedThreadPageRejection::InvalidThreadId)
        );
        assert!(ids.is_empty());
