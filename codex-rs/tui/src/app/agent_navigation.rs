...
impl AgentNavigationState {
    ...
    /// treated as `/root` so its descendants can render under it.
    pub(crate) fn picker_tree_prefixes(
        ...
        let path_by_thread_id = ordered_threads
            .into_iter()
            .map(|(thread_id, entry)| {
                let path = if Some(thread_id) == primary_thread_id {
                    Some("/root".to_string())
                } else {
        ...
        for root_thread_id in roots {
            visit(root_thread_id, &[], &children_by_parent, &mut prefixes);
        }
        prefixes
    }
