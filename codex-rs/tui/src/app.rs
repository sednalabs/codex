...
impl App {
    ...
    fn apply_active_thread_session_state_to_config(&mut self, config: &mut Config) {
        ...
        if let Err(err) = config.permissions.approval_policy.set(approval_policy) {
            tracing::warn!(%err, "failed to sync approval_policy from active thread session");
        ...
        {
            tracing::warn!(%err, "failed to sync sandbox_policy from active thread session");
...
mod tests {
    ...
    async fn windows_sandbox_transition_resume_waits_for_active_popup() {
        ...
        assert_no_user_turn_op(&mut op_rx);
...
"#;
    ...
    fn assert_no_user_turn_op(op_rx: &mut tokio::sync::mpsc::UnboundedReceiver<Op>) {
        while let Ok(op) = op_rx.try_recv() {
            assert!(
                !matches!(op, Op::UserTurn { .. }),
                "unexpected UserTurn op: {op:?}"
            );
        }
    }
    ...
    #[tokio::test]
    async fn refresh_in_memory_config_from_disk_preserves_active_thread_session_state() -> Result<()>
    {
        let mut app = make_test_app().await;
        let codex_home = tempdir()?;
        ...
        assert_eq!(
            (
                ...
                app.config.permissions.approval_policy.value(),
                app.config.approvals_reviewer,
                app.config.permissions.sandbox_policy.get().clone(),
                app.config.model_reasoning_effort,
            ),
            (
                session_cwd,
    ...
    #[tokio::test]
    async fn fresh_session_config_uses_current_session_state() {
        let mut app = make_test_app().await;
    ...
    async fn confirm_subagent_exit_returns_to_primary_thread() -> Result<()> {
        ...
        let subagent_thread_id = ThreadId::new();
        let primary_thread_id = ThreadId::new();
        let primary_session = test_thread_session(
            primary_thread_id,
            PathBuf::from(test_path_display("/tmp/main")),
        );
        let subagent_session = test_thread_session(
            subagent_thread_id,
            PathBuf::from(test_path_display("/tmp/agent")),
        );
        app.active_thread_id = Some(subagent_thread_id);
        app.primary_thread_id = Some(primary_thread_id);
