...
fn function_call_image_detail_from_tool_hint(detail: Option<&str>) -> Option<ImageDetail> {
    match detail {
        None => Some(DEFAULT_IMAGE_DETAIL),
        Some("high") => Some(ImageDetail::High),
        Some("original") => Some(ImageDetail::Original),
        // Current protocol only carries high/original. Treat legacy low hints as
        // unspecified rather than smuggling an unsupported value into the model payload.
        Some("low") => None,
        _ => None,
    }
}
...
mod tests {
    ...
    #[test]
    fn thread_inference_identity_validates_construction_and_deserialization() -> Result<()> {
        let identity = ThreadInferenceIdentity::new(
            "configured-alias",
            "test-provider",
        ...
        assert_eq!(
            serde_json::from_value::<ThreadInferenceIdentity>(serde_json::to_value(&identity)?)?,
            identity
        );

        assert_eq!(
            ThreadInferenceIdentity::new("  ", "provider", None),
            Err(ThreadInferenceIdentityValidationError::EmptyModel)
        );
        assert_eq!(
            ThreadInferenceIdentity::new("model", "\t", None),
            Err(ThreadInferenceIdentityValidationError::EmptyModelProviderId)
        );
        for raw in [
            serde_json::json!({
                "model": " ",
            ...
            assert!(serde_json::from_value::<ThreadInferenceIdentity>(raw).is_err());
        }
        Ok(())
    }
