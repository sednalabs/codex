...
fn run_conversation_summary_test<F, Fut>(test_body: F) -> Result<()>
where
    F: FnOnce() -> Fut + Send + 'static,
    Fut: Future<Output = Result<()>> + Send + 'static,
{
    let handle = std::thread::Builder::new()
        .name("conversation-summary-test".to_string())
        .stack_size(16 * 1024 * 1024)
        .spawn(move || {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .expect("conversation summary test runtime should build")
                .block_on(test_body())
        })?;
...
fn expected_summary(conversation_id: ThreadId, path: PathBuf) -> Result<ConversationSummary> {
...
fn get_conversation_summary_by_thread_id_reads_pathless_store_thread() -> Result<()> {
    run_conversation_summary_test(|| async {
        let codex_home = TempDir::new()?;
