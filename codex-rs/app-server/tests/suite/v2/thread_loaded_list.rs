...
#[tokio::test]
async fn thread_loaded_list_omitted_limit_returns_all_loaded_thread_ids() -> Result<()> {
    const LOADED_THREAD_COUNT: usize = 101;
    ...
    let expected = start_loaded_threads(&mut mcp, LOADED_THREAD_COUNT).await?;

    // The original, unfiltered API is not pagination-aware: an omitted limit must include every
    // loaded id rather than silently dropping those beyond the newer filtered page boundary.
    let list_id = mcp
        .send_thread_loaded_list_request(ThreadLoadedListParams::default())
        .await?;
    let ThreadLoadedListResponse {
...
#[tokio::test]
async fn thread_loaded_list_explicit_limit_above_100_is_not_clamped() -> Result<()> {
    const EXPLICIT_LIMIT: usize = 101;
    ...
    let expected = start_loaded_threads(&mut mcp, EXPLICIT_LIMIT).await?;

    let list_id = mcp
        .send_thread_loaded_list_request(ThreadLoadedListParams {
            cursor: None,
            limit: Some(EXPLICIT_LIMIT as u32),
            ancestor_thread_id: None,
        })
        .await?;
    let ThreadLoadedListResponse {
...
#[tokio::test]
async fn thread_loaded_list_filters_and_paginates_loaded_spawn_descendants() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    MockResponsesConfig::new(&server.uri()).write(codex_home.path())?;
    ...
    let direct_descendant_thread_id = create_fake_parented_rollout_with_source(
        codex_home.path(),
        "2026-01-07T00-00-05",
        "2026-01-07T00:00:05Z",
        "Saved direct descendant message",
        Some("mock_provider"),
        /*git_info*/ None,
        SessionSource::SubAgent(SubAgentSource::ThreadSpawn {
            parent_thread_id: root_thread_uuid,
    ...
    let direct_descendant_thread_uuid = ThreadId::from_string(&direct_descendant_thread_id)?;
    ...
    for (parent_thread_id, child_thread_id) in [
        ...
        (root_thread_uuid, direct_descendant_thread_uuid),
        (
            unrelated_root_thread_uuid,
    ...
    // not materialize this historical subtree merely to find the loaded descendants.
    ...
    // child itself. The direct descendant provides a second filtered result for pagination.
    for thread_id in [
        &root_thread_id,
        &grandchild_thread_id,
        &direct_descendant_thread_id,
    ...
    let mut expected = vec![grandchild_thread_id.clone(), direct_descendant_thread_id.clone()];
    expected.sort();
    ...
    assert_eq!(next_cursor, None);
    assert!(ancestor_filter_applied);
    assert!(!first_page.contains(&child_thread_id));
    assert!(!first_page.contains(&unrelated_root_thread_id));
    assert!(!first_page.contains(&unrelated_child_thread_id));
    assert!(!second_page.contains(&child_thread_id));
    assert!(!second_page.contains(&unrelated_root_thread_id));
    assert!(!second_page.contains(&unrelated_child_thread_id));

    // An oversized public limit is clamped before it reaches the filtered manager path. The same
    // bounded path handles an omitted limit for this newer ancestor-filtered request form.
...
async fn start_loaded_threads(mcp: &mut TestAppServer, count: usize) -> Result<Vec<String>> {
    let mut thread_ids = Vec::with_capacity(count);
    for _ in 0..count {
        thread_ids.push(start_thread(mcp).await?);
    }
    thread_ids.sort();
    Ok(thread_ids)
}
