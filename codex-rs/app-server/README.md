...
- `turn/completed` — `{ turn, finalModel, modelSnapshot }` where `turn.status` is `completed`, `interrupted`, or `failed`; execution identity fields are `null` when the Responses API does not report them, and failures carry `{ error: { message, codexErrorInfo?, additionalDetails? } }`.
