...
fn detect_repo_canonicalizes_relative_cwd() {
    ...
    assert_eq!(
        find_repo_root(Some(relative_nested)).expect("find repository root"),
        Some(fs::canonicalize(repo_root).expect("canonicalize repository root"))
    );
}
...
async fn detect_repo_canonicalizes_symlinked_nested_cwd_before_containment_checks() {
    ...
    let local_settings = repo_root
        .join(EXTERNAL_AGENT_DIR)
        .join(REPO_EXTERNAL_AGENT_LOCAL_SETTINGS_FILE);
    ...
    assert_eq!(
        find_repo_root(Some(&linked_cwd)).expect("find repository root"),
        Some(fs::canonicalize(&repo_root).expect("canonicalize repository root"))
    );
...
async fn import_repo_mcp_rejects_symlinked_source_files() {
    for source_name in [
        REPO_EXTERNAL_AGENT_MCP_CONFIG_FILE,
        REPO_EXTERNAL_AGENT_PROJECT_CONFIG_FILE,
    ] {
        let root = TempDir::new().expect("create tempdir");
        let repo_root = root.path().join("repo");
        let external_mcp = root.path().join("external-mcp.json");
