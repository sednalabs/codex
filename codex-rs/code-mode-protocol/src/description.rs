...
- Native computer-use tools return a typed `{ content, success }` object in code mode. Forward an `input_image` item with `image(result.content.find((item) => item.type === "input_image"))`; do not pass the complete result to `text()` because it contains the inline image data.
