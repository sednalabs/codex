...
pub struct ThreadMetadata {
    ...
    /// Whether a settings snapshot has established configured-identity provenance.
    pub configured_identity_seen: bool,
...
impl ThreadMetadata {
    ...
    pub fn diff_fields(&self, other: &Self) -> Vec<&'static str> {
        ...
        if self.configured_identity_seen != other.configured_identity_seen {
            diffs.push("configured_identity_seen");
        }
...
impl ThreadRow {
    pub(crate) fn try_from_row(row: &SqliteRow) -> Result<Self> {
        Ok(Self {
            ...
            configured_identity_seen: row.try_get("configured_identity_seen")?,
            cwd: row.try_get("cwd")?,
