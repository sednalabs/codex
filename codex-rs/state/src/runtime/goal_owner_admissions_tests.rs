...
#[tokio::test]
async fn recovery_release_requires_the_current_owner_installation_capability() {
    let codex_home = unique_temp_dir();
    let sqlite = crate::SqliteConfig::new_for_testing(codex_home.as_path().abs());
    let runtime_a = StateRuntime::init(sqlite.clone(), "test-provider".to_string())
        .await
        .expect("initialize first owning runtime");
    let recovery_capability_a = runtime_a
        .goal_owner_recovery_capability()
        .expect("first owner receives recovery capability");
    let record = runtime_a
        .goal_owner_admissions()
        .observe_denial(&observation(
            ThreadId::new(),
            "recovery-capability",
            "request-recovery-capability",
            Utc::now() - chrono::Duration::seconds(1),
            GoalOwnerAdmissionPhase::Pending,
        ))
        .await
        .expect("record pending admission");
    let claim_id = runtime_a
        .goal_owner_admissions()
        .claim_dispatch(
        ...
        .await
        .expect("claim dispatch")
        .expect("pending generation is claimable");
    runtime_a.close().await;
    drop(runtime_a);

    let runtime_b = StateRuntime::init(sqlite, "test-provider".to_string())
        .await
        .expect("initialize replacement owning runtime");
    let recovery_capability_b = runtime_b
        .goal_owner_recovery_capability()
        .expect("replacement owner receives recovery capability");
    let error = runtime_b
        .goal_owner_admissions()
        .release_dispatch_claim_after_owner_recovery(
            &record.authority,
            claim_id,
            &recovery_capability_a,
        )
        .await
        .expect_err("a previous installation capability must be rejected");
    ...
    assert!(
        runtime_b
            ...
            .release_dispatch_claim_after_owner_recovery(
                &record.authority,
                claim_id,
                &recovery_capability_b,
            )
            .await
            .expect("current installation may recover a predecessor claim")
    );
