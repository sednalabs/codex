...
impl ExternalAgentConfigService {
    ...
    pub(crate) fn effective_source_settings(
        ...
        self.ensure_repo_settings_sources(scope)?;
    ...
    pub(crate) fn build_mcp_config(
        ...
        self.ensure_repo_mcp_sources(scope)?;
    ...
    pub(crate) fn repo_agents_md_source_groups(
        ...
        for source in self.source.repo_instruction_sources(repo_root) {
            ensure_migration_path(repo_root, &source)?;
        }
        self.source.repo_instruction_source_groups(repo_root)
    }

    pub(crate) fn ensure_scope_migration_path(
        &self,
        scope: &MigrationScope,
        path: &Path,
    ) -> io::Result<()> {
    ...
    pub(crate) fn ensure_repo_hook_sources(&self, scope: &MigrationScope) -> io::Result<()> {
        let Some(repo_root) = scope.repo_root() else {
            return Ok(());
        };
        for source in self.source.repo_hook_sources(repo_root) {
            ensure_migration_path(repo_root, &source)?;
        }
        Ok(())
    }

    fn ensure_repo_settings_sources(&self, scope: &MigrationScope) -> io::Result<()> {
        let Some(repo_root) = scope.repo_root() else {
            return Ok(());
        };
        for source in self.source.repo_settings_sources(repo_root) {
            ensure_migration_path(repo_root, &source)?;
        }
        Ok(())
    }

    fn ensure_repo_mcp_sources(&self, scope: &MigrationScope) -> io::Result<()> {
        let Some(repo_root) = scope.repo_root() else {
            return Ok(());
        };
        for source in self.source.repo_mcp_sources(repo_root) {
            ensure_migration_path(repo_root, &source)?;
        }
        Ok(())
    }
    ...
    fn import_subagents(&self, cwd: Option<&Path>) -> io::Result<Vec<String>> {
        ...
        self.ensure_scope_migration_path(&scope, &source_agents)?;
        self.ensure_scope_migration_path(&scope, &target_agents)?;
    ...
    fn import_skills(&self, cwd: Option<&Path>) -> io::Result<Vec<String>> {
        ...
        validate_skill_import_root(&source_skills)?;
    ...
    fn import_agents_md(&self, cwd: Option<&Path>) -> io::Result<Option<(String, String)>> {
        ...
        let (source_agents_md, target_agents_md) = match scope {
            MigrationScope::Repository { root } => {
                ...
                ensure_migration_path(&root, &target_agents_md)?;
