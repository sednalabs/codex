...
| Per-turn developer bundle          | sandbox and approval guidance, collaboration-mode instructions, memory instructions, realtime start/end hints, personality text, apps/connectors, skills, plugins, commit attribution | per-turn assembly in `codex-rs/core/src/session/mod.rs` plus runtime feature/config plumbing | runtime/harness owners, plugin and skill authors, user config for feature toggles         | Mode, runtime wiring, tool and connector availability, short-lived execution scaffolding |
