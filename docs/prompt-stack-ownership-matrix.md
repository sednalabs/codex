...
| Per-turn developer bundle          | sandbox and approval guidance, collaboration-mode instructions, memory instructions, realtime start/end hints, personality text, apps/connectors, skills, plugins, git-attribution world state | per-turn assembly in `codex-rs/core/src/session/mod.rs` plus extension contributions, including `codex-rs/ext/git-attribution` | runtime/harness owners, extension/plugin/skill authors, authenticated workspace policy owners | Mode, runtime wiring, tool and connector availability, short-lived execution scaffolding |
