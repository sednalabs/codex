...
- If a tag push is only the duplicate event produced after the `main` release has already published
  the same tag for the same target commit, the workflow treats it as an idempotent skip instead of
  rebuilding or reporting a false release failure.
