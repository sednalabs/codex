...
| Deferred bare native dynamic tools | Deferred ordinary dynamic tools require a namespace | Bare native computer-use tools may defer-load for `tool_search`; capability-bearing native tools reload a loaded thread before resume. | yes | `validate_dynamic_tools_accepts_deferred_bare_native_computer_use_tool`; `capability_bearing_native_dynamic_tool_requires_thread_reload` |
