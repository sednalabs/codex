...
| `list_agents` | absent | Direct-child inventory tool with live status, provenance (`identity_source`), and effective-setting metadata | yes | `list_agents_returns_direct_children_with_live_inventory`; `list_agents_respects_optional_id_filter`; `list_agents_id_filter_returns_not_found_entries_for_missing_or_invisible_ids`; `codex.core-subagent-surface-targeted`; `codex.core-multi-agent-orchestration-targeted` |
