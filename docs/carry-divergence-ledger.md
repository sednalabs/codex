...
- Local lifecycle eligibility is classified under per-thread custody shared by create/resume, shutdown/discard, archive/unarchive, and delete. Materialized active or archived rollout evidence outranks a live-recorder snapshot, so an archived live recorder cannot be mistaken for active authority eligibility.
- Exact lifecycle selectors prove archived-live and ambiguous-state classification, archive/unarchive/delete/discard transition ordering, unrelated-thread progress, and weak custody-registry cleanup in the inference-identity lane.
