...
- Spawned agents receive bounded `<subagent_runtime_identity>` developer
  fragments before provider requests. The shared model-visible projection
  keeps configured settings separate from the latest normalized turn request,
  preserves provenance for both receipts, and reports truncation and omitted
  fields without exposing unbounded internal strings. Canonical initial context
  rebuilds the current projection after compaction, while settings or request
  changes append a latest-fragment-wins update without rewriting cached
  history. Full-history forks remove inherited parent fragments, and raw
  developer-item injection reserves the marker namespace, so neither task text
  nor client-supplied developer items can become routing truth.
