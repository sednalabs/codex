...
- Local lifecycle eligibility is classified under per-thread custody shared by create/resume, discard, archive/unarchive, and delete. Unbounded shutdown drain stays outside custody; its short removal commit takes custody, and token-checked cleanup prevents a late drain from removing a replacement recorder.
- Materialized active or archived rollout evidence outranks a live-recorder snapshot, and an ever-materialized recorder whose rollout vanished fails closed as missing. Exact lifecycle selectors prove degraded archived-live and delete-preflight states, transition ordering, unrelated-thread progress, cancellation reuse, and weak custody-registry cleanup in the inference-identity lane.
