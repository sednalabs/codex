...
| `schema/typescript/v2/ThreadStartParams.ts`, `ThreadResumeParams.ts`, `ThreadForkParams.ts` | `baseInstructions` / `developerInstructions` are typed as `string | null` | Upstream types preserve the explicit-null override contract as `string | null | null` | The generated downstream types collapse the explicit-null semantics that upstream uses to distinguish “field omitted” from “explicitly clear / use built-in behavior”. |
