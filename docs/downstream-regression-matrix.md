...
| TUI transcript compact detail mode keeps `Ctrl+T` usable for prompt/agent review while preserving verbose transcript audit mode | `codex.tui-transcript-viewport-targeted`; remote `validation-lab` `ui-protocol` lane set | `transcript_overlay_compact_mode_uses_display_lines`; `transcript_overlay_state_round_trips_scroll_selection_and_mode`; `transcript_overlay_renders_live_tail`; `transcript_overlay_footer_status_snapshot`; `UserHistoryCell` compact transcript coverage; `HookCell` compact transcript coverage |
