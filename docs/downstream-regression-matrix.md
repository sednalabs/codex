...
| MCP OAuth device login for headless Streamable HTTP servers | `codex.mcp-device-login-targeted`; remote `validation-lab` targeted profile | `discover_streamable_http_oauth_returns_normalized_scopes`; `device_login_polls_until_authorized`; `mcp_login_parses_device_auth_flag`; remote branch build for the Linux `codex` preview binary |
