...
- `wait_agent` supports `return_when=any|all` and returns `requested_ids`, `pending_ids`, `completion_reason`, and `timed_out`. In the v2 surface, callers may omit `targets` when they intentionally want to wait only for mailbox activity or timeout.
