...
- V2 `send_message` returns a queue-acceptance receipt with the canonical target and its effective model, provider, reasoning effort, and service tier. `handoff_state: queued` does not claim that the target agent acknowledged or completed the work.
