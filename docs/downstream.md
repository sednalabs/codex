...
- The built-in `explorer` role now locks only `medium` reasoning effort and preserves the caller's current model/provider, so codebase-question lanes keep an availability-safe runtime model while still getting the intended cheaper review depth when that effort is supported.
