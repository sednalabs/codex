...
- For helper-backed waits launched by orchestrators (for example `exec_command` / `write_stdin`), use `wait_until_terminal=true` so waiting happens in the tool layer instead of transcript polling loops.
