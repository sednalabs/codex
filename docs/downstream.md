...
- Replay drains queued slash commands in order until one starts a new task, so non-task commands
  (such as `/plan` replayed after `/compact`) do not stall queued message follow-ups.
