...
- Bare `browser_observe` and `browser_step` dynamic tools are also promoted to canonical Codex function tools with adapter `browser`; the current TUI provider registry routes them to a browser bridge when configured, and CLI/TUI sessions auto-advertise those browser tools when a local browser provider is configured.
