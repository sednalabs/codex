...
### Commit-scoped check runs

- `gh api repos/{owner}/{repo}/commits/{sha}/check-runs -X GET -f per_page=100`

Used as a current-head-scoped signal to separate in-flight checks for the latest commit from stale older-head failures.
