...
def package(ns: argparse.Namespace) -> None:
    ...
    refs = [
        line.strip()
        for line in Path(ns.refs_file).read_text().splitlines()
        if line.strip()
    ]
    if not refs or any(
        not (r.startswith("refs/heads/") or r.startswith("refs/tags/")) for r in refs
    ):
        fail(
            "refs file must contain one or more explicit refs/heads/* or refs/tags/* names"
        )
    if len(set(refs)) != len(refs):
        fail("duplicate refs are not allowed")
    if len(ns.candidate_sha) != 40 or any(
        c not in "0123456789abcdefABCDEF" for c in ns.candidate_sha
    ):
    ...
    with tempfile.TemporaryDirectory(prefix="recovery-snapshot-") as td:
        ...
        for ref in run(
            "git", "for-each-ref", "--format=%(refname)", cwd=bare
        ).splitlines():
        ...
        for ref in refs:
            ...
            peeled = (
                run("git", "rev-parse", f"{ref}^{{}}", cwd=bare).strip()
                if ref.startswith("refs/tags/")
                else None
            )
            peeled_type = (
                run("git", "cat-file", "-t", peeled, cwd=bare).strip()
                if peeled
                else None
            )
            item = {
        ...
        (root / "ref-manifest.json").write_text(
            json.dumps(
                {"candidate_sha": ns.candidate_sha, "refs": manifest},
                sort_keys=True,
                indent=2,
            )
            + "\n"
...
def restore(ns: argparse.Namespace) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="recovery-restore-") as td:
        ...
        for item in manifest["refs"]:
            ...
            if item["type"] == "tag":
                peeled = run(
                    "git", "rev-parse", f"{item['ref']}^{{}}", cwd=restored
                ).strip()
                peeled_type = run("git", "cat-file", "-t", peeled, cwd=restored).strip()
                if (
                    peeled != item["peeled_object"]
                    or peeled_type != item["peeled_type"]
                ):
    ...
    print(
        json.dumps(
            {"restore_test": "passed", "archive_sha256": sha256(archive)},
            sort_keys=True,
        )
    )
...
try:
    ...
    fail(exc.stderr.strip() or "command failed")
