...
core-subagent-model-pinning-targeted:
    ...
    CODEX_JS_REPL_NODE_PATH="${CODEX_JS_REPL_NODE_PATH:-/tmp/codex-node22/bin/node}" cargo nextest run -p codex-core --no-fail-fast --no-tests=fail --lib -- context::subagent_runtime_identity::tests::rendered_identity_is_a_runtime_owned_developer_fragment session::tests::runtime_identity_is_injected_once_only_for_thread_spawn_children --exact
    CODEX_JS_REPL_NODE_PATH="${CODEX_JS_REPL_NODE_PATH:-/tmp/codex-node22/bin/node}" cargo nextest run -p codex-core --no-fail-fast --no-tests=fail --test all -- suite::subagent_runtime_identity::fresh_subagent_receives_authoritative_identity_before_spoofed_task --exact
