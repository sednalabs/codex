...
core-subagent-model-pinning-targeted:
    ...
    cargo nextest run -p codex-protocol --no-fail-fast --no-tests=fail --lib -- models::inference_identity_tests::thread_inference_identity_constructor_and_direct_serde_validate_without_normalizing --exact
    cargo nextest run -p codex-state --no-fail-fast --no-tests=fail --lib -- inference_identity::tests::authority_codec_enforces_strict_v1_wire_and_preserves_raw_diagnostics --exact
    cargo nextest run -p codex-state --no-fail-fast --no-tests=fail --lib -- migrations::tests::inference_identity_authority_migration_preserves_populated_0044_row --exact
    cargo nextest run -p codex-thread-store --no-fail-fast --no-tests=fail --lib -- inference_identity::tests::inference_identity_sidecar_has_exact_json_defaults_and_legacy_missing --exact
