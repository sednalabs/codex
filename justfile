...
core-subagent-model-pinning-targeted:
    ...
    CODEX_JS_REPL_NODE_PATH="${CODEX_JS_REPL_NODE_PATH:-/tmp/codex-node22/bin/node}" cargo nextest run -p codex-core --no-fail-fast --no-tests=fail --lib -- context::subagent_runtime_identity::tests::current_match_requires_exact_model_visible_projection session::tests::build_initial_context_adds_one_runtime_identity_only_for_thread_spawn_children session::tests::runtime_identity_is_cache_stable_and_refreshes_after_settings_change --exact
