...
thread-inference-identity-targeted:
    ...
    RUST_MIN_STACK="${RUST_MIN_STACK:-{{ rust_min_stack }}}" cargo nextest run -p codex-thread-store --no-fail-fast --no-tests=fail --lib -- local::lifecycle::tests::lifecycle_custody_regression_suite --exact
