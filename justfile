...
core-turn-admission-custody-targeted:
    ...
    RUST_MIN_STACK="${RUST_MIN_STACK:-{{ rust_min_stack }}}" CODEX_JS_REPL_NODE_PATH="${CODEX_JS_REPL_NODE_PATH:-/tmp/codex-node22/bin/node}" cargo nextest run -p codex-core --no-fail-fast --no-tests=fail --lib -- session::tests::successful_late_steer_reopens_same_turn_without_duplicate_lifecycle session::tests::turn_local_continuation_preserves_custody_for_owner_markers session::tests::turn_local_continuation_rejects_stale_identity_and_next_turn_mailbox session::tests::turn_local_continuation_rejects_terminal_error_and_keeps_pending_input session::tests::cancelled_turn_local_continuation_preserves_pending_input session::tests::cancellation_during_late_steer_decision_gap_prevents_continuation --exact
    RUST_MIN_STACK="${RUST_MIN_STACK:-{{ rust_min_stack }}}" CODEX_JS_REPL_NODE_PATH="${CODEX_JS_REPL_NODE_PATH:-/tmp/codex-node22/bin/node}" cargo nextest run -p codex-core --no-fail-fast --no-tests=fail --test all -- suite::pending_input::late_steer_reopens_regular_turn_at_finalization_boundary --exact
