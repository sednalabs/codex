...
core-subagent-model-pinning-targeted:
    ...
    RUST_MIN_STACK="${RUST_MIN_STACK:-{{ rust_min_stack }}}" cargo nextest run -p codex-core --no-fail-fast --no-tests=fail --lib -- agent::control::tests::spawn_agent_can_fork_parent_thread_history_with_sanitized_items agent::control::tests::ensure_v2_agent_loaded_reloads_registered_unloaded_agent --exact
