...
core-subagent-model-pinning-targeted:
    RUST_MIN_STACK="${RUST_MIN_STACK:-{{ rust_min_stack }}}" CODEX_JS_REPL_NODE_PATH="${CODEX_JS_REPL_NODE_PATH:-/tmp/codex-node22/bin/node}" cargo nextest run -p codex-core --no-fail-fast --no-tests=fail --lib -- session::tests::turn_inference_identity_captures_final_request_service_tier session::tests::real_turn_construction_publishes_configured_and_request_identity agent::identity::tests::projection_caps_json_and_xml_after_escaping agent::identity::tests::configured_and_turn_request_provenance_stay_separate agent::control::tests::identity_receipt_survives_settings_update_eviction_and_reload --exact
    RUST_MIN_STACK="${RUST_MIN_STACK:-{{ rust_min_stack }}}" cargo nextest run -p codex-core --no-fail-fast --no-tests=fail --lib -- tools::handlers::multi_agents_spec::tests::spawn_agent_tool_v2_requires_task_name_and_lists_visible_models --exact
    RUST_MIN_STACK="${RUST_MIN_STACK:-{{ rust_min_stack }}}" CODEX_JS_REPL_NODE_PATH="${CODEX_JS_REPL_NODE_PATH:-/tmp/codex-node22/bin/node}" cargo nextest run -p codex-core --no-fail-fast --no-tests=fail --test all -- suite::subagent_notifications::spawn_agent_requested_model_and_reasoning_override_inherited_settings_without_role suite::subagent_notifications::spawn_agent_role_overrides_requested_model_and_reasoning_settings suite::subagent_notifications::list_agents_returns_model_visible_identity_receipts --exact
    RUST_MIN_STACK="${RUST_MIN_STACK:-{{ rust_min_stack }}}" cargo nextest run -p codex-state -p codex-thread-store --no-fail-fast --no-tests=fail --lib -- extract::tests::thread_settings_replace_complete_configured_identity_and_clear_effort local::read_thread::tests::read_thread_keeps_complete_indexed_identity_during_rollout_overlay local::read_thread::tests::read_thread_keeps_complete_indexed_identity_for_malformed_rollout thread_metadata_sync::tests::settings_events_replace_complete_configured_identity_and_preserve_clears types::tests::thread_metadata_patch_round_trips_optional_clears types::tests::legacy_reasoning_effort_null_remains_a_noop --exact
...
thread-inference-identity-targeted:
    ...
    RUST_MIN_STACK="${RUST_MIN_STACK:-{{ rust_min_stack }}}" cargo nextest run -p codex-thread-store --no-fail-fast --no-tests=fail --lib -- local::lifecycle::tests::classifies_missing_unmaterialized_materialized_and_archived_live_threads --exact
    RUST_MIN_STACK="${RUST_MIN_STACK:-{{ rust_min_stack }}}" cargo nextest run -p codex-thread-store --no-fail-fast --no-tests=fail --lib -- local::lifecycle::tests::archive_transition_completes_before_queued_classification --exact
    RUST_MIN_STACK="${RUST_MIN_STACK:-{{ rust_min_stack }}}" cargo nextest run -p codex-thread-store --no-fail-fast --no-tests=fail --lib -- local::lifecycle::tests::unarchive_transition_completes_before_queued_classification --exact
    RUST_MIN_STACK="${RUST_MIN_STACK:-{{ rust_min_stack }}}" cargo nextest run -p codex-thread-store --no-fail-fast --no-tests=fail --lib -- local::lifecycle::tests::custody_is_per_thread_and_prunes_idle_entries --exact
    RUST_MIN_STACK="${RUST_MIN_STACK:-{{ rust_min_stack }}}" cargo nextest run -p codex-thread-store --no-fail-fast --no-tests=fail --lib -- local::lifecycle::tests::rejects_simultaneous_active_and_archived_materialization --exact
    RUST_MIN_STACK="${RUST_MIN_STACK:-{{ rust_min_stack }}}" cargo nextest run -p codex-thread-store --no-fail-fast --no-tests=fail --lib -- local::lifecycle::tests::delete_transition_completes_before_queued_classification --exact
    RUST_MIN_STACK="${RUST_MIN_STACK:-{{ rust_min_stack }}}" cargo nextest run -p codex-thread-store --no-fail-fast --no-tests=fail --lib -- local::lifecycle::tests::discard_transition_completes_before_queued_classification --exact
