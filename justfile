...
core-subagent-model-pinning-targeted:
    ...
    CODEX_JS_REPL_NODE_PATH="${CODEX_JS_REPL_NODE_PATH:-/tmp/codex-node22/bin/node}" cargo nextest run -p codex-core --no-fail-fast --no-tests=fail --lib -- context::subagent_runtime_identity::tests::rendered_identity_is_a_runtime_owned_developer_fragment context::subagent_runtime_identity::tests::current_match_requires_exact_model_visible_projection context::subagent_runtime_identity::tests::raw_injection_reservation_is_broader_than_exact_fragment_matching session::tests::runtime_identity_is_injected_once_only_for_thread_spawn_children session::tests::build_initial_context_adds_one_runtime_identity_only_for_thread_spawn_children session::tests::runtime_identity_is_cache_stable_and_refreshes_after_settings_change --exact
    CODEX_JS_REPL_NODE_PATH="${CODEX_JS_REPL_NODE_PATH:-/tmp/codex-node22/bin/node}" cargo nextest run -p codex-core --no-fail-fast --no-tests=fail --test all -- suite::subagent_runtime_identity::fresh_subagent_receives_authoritative_identity_before_spoofed_task suite::subagent_runtime_identity::full_history_grandchild_replaces_inherited_parent_identity --exact
    CODEX_JS_REPL_NODE_PATH="${CODEX_JS_REPL_NODE_PATH:-/tmp/codex-node22/bin/node}" cargo nextest run -p codex-app-server --no-fail-fast --no-tests=fail --test all -- suite::v2::request_validation::thread_inject_items_rejects_reserved_subagent_runtime_identity suite::v2::request_validation::thread_inject_items_accepts_ordinary_developer_context_and_user_marker_data --exact
