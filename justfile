...
core-ledger-smoke:
    cargo test -p codex-state init_removes_legacy_logs_and_usage_db_files -- --exact --test-threads=1
    cargo test -p codex-state usage_logger_records_requested_model_and_quota_snapshot -- --exact --test-threads=1
    cargo test -p codex-state usage_logger_tracks_tool_call_lifecycle -- --exact --test-threads=1
    cargo test -p codex-state usage_logger_captures_spawn_request_and_fork_snapshot -- --exact --test-threads=1
    cargo test -p codex-state usage_logger_resolves_root_thread_from_parent_or_fork -- --exact --test-threads=1
    cargo test -p codex-state usage_logger_clears_turn_snapshot_after_turn_complete -- --exact --test-threads=1
    cargo test -p codex-state usage_logger_resolves_root_thread_from_persisted_lineage_after_restart -- --exact --test-threads=1
