...
[no-cd]
prerelease-main:
    gh workflow run sedna-release.yml --repo sednalabs/codex --ref main -f channel=prerelease -f draft=false -f macos_release_mode=preview -f allow_markerless_prerelease=true
...
blocking-waits-unified-exec-targeted:
    ...
    RUST_MIN_STACK="${RUST_MIN_STACK:-{{ rust_min_stack }}}" CODEX_JS_REPL_NODE_PATH="${CODEX_JS_REPL_NODE_PATH:-/tmp/codex-node22/bin/node}" cargo nextest run -j 1 -p codex-core --lib -- tools::handlers::unified_exec::tests::blocking_wait_window_accepts_long_budget_and_caps_at_two_hours --exact
...
# Focused P4 browser-provider and exec dependency slice. Keep this lane limited
# to packages present in the exact candidate so unrelated newer TUI surfaces do
# not contaminate the provider proof.
native-computer-use-tool-registry-targeted:
    cargo test --locked -p codex-browser-computer-use --lib -- --test-threads=1
    cargo test --locked -p codex-exec --lib -- --test-threads=1
