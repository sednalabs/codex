...
p3-terminal-consumer-probe:
    RUST_MIN_STACK={{ rust_min_stack }} CODEX_JS_REPL_NODE_PATH="${CODEX_JS_REPL_NODE_PATH:-/tmp/codex-node22/bin/node}" cargo nextest run -p codex-core --no-fail-fast --lib -E 'test(/(config_schema_matches_fixture|spawn_agent_creates_thread_and_sends_prompt|send_inter_agent_communication_without_turn_queues_message_without_triggering_turn|multi_agent_v2_spawn_returns_path_and_send_message_accepts_relative_path|multi_agent_v2_wait_agent_returns_summary_for_mailbox_activity|multi_agent_v2_interrupt_agent_accepts_task_name_target|multi_agent_v2_followup_task_completion_notifies_parent_on_every_turn)/)'
    RUST_MIN_STACK={{ rust_min_stack }} CODEX_JS_REPL_NODE_PATH="${CODEX_JS_REPL_NODE_PATH:-/tmp/codex-node22/bin/node}" cargo nextest run -p codex-core --no-fail-fast --test all -E 'test(/(steer_interrupts_wait_agent_and_is_sent_in_follow_up_request|multi_agent_v2_spawn_sends_agent_message_to_child|plaintext_multi_agent_v2_completion_sends_agent_message)/)'
