...
# Validation-only recovery of the generated stable protocol export selected by
# the P5 consumer probe. The generated bytes stay in runner temp and are
# emitted as fixed-size log chunks for bounded artifact recovery.
app-server-v2-contract-targeted:
    #!/usr/bin/env bash
    set -euo pipefail
    scratch="${RUNNER_TEMP:?}/p5-app-mcp-schema-artifact"
    rm -rf "${scratch}"
    mkdir -p "${scratch}"
    python3 app-server-protocol/scripts/write_schema_fixtures.py --schema-root "${scratch}"
    blob="${scratch}/precomputed/app-server-exports-stable.json.zst"
    test -s "${blob}"
    echo "P5_SCHEMA_ARTIFACT_SHA256=$(sha256sum "${blob}" | awk '{print $1}')"
    echo "P5_SCHEMA_ARTIFACT_SIZE=$(wc -c < "${blob}")"
    echo "P5_SCHEMA_ARTIFACT_B64_BEGIN"
    base64 -w4096 "${blob}"
    echo
    echo "P5_SCHEMA_ARTIFACT_B64_END"
